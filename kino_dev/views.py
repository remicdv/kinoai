...
class LoginView(generic.View):
    template_name='kino_dev/login.html'
    def post(self, request):
        ...
        if user is not None:
            if user.is_active and user.is_authenticated == True:
                login(request, user)
                return HttpResponseRedirect('/kino_dev')
            else:
                return render(request, "kino_dev/login.html", {'msg':"Inactive user"})
        else:
            return render(request, "kino_dev/login.html", {'msg':"Username or password invalid"})

        return render(request, "kino_dev/login.html")
...
def signin_view(request):
    if request.method == "POST":
        ...
        if not (left+answer)==right:
            return render(request, "kino_dev/sign_in.html", {'msg':'Check your math !', 'left_part':random.randint(1,10), 'right_part':random.randint(1,20)})
        if username:
            ...
            if len(user_set) == 0:
                ...
                return render(request, "kino_dev/sign_in.html", {'msg':'Username already used', 'left_part':random.randint(1,10), 'right_part':random.randint(1,20)})

    return render(request, "kino_dev/sign_in.html", {'left_part':random.randint(1,10), 'right_part':random.randint(1,20)})
    ...
    for root, dirs, files in os.walk(startpath):
        for name in dirs:
            ...
            if (name != 'mpd' and name != 'images') and not ret:
                d = dir_path.split('/kino_dev/data/')
                list.append(d[1])
...
def extractImagesAndDelete(list, len_old):
    # print(list)
    if len(list) != len_old:
        img_folder_path = os.path.join(settings.MEDIA_ROOT, "kino_dev/index_images/")
        for p in FolderPath.objects.all():
            if p.path not in list:
                ...
                if len(obj) == 0:
                    with open(os.path.join(settings.MEDIA_ROOT, 'kino_dev/data/', p.path)+'/detections.json') as f:
                        data = json.load(f)
...
class IndexView(generic.ListView):
    ...
    template_name = 'kino_dev/index.html'
    ...
    def get(self, request):
        ...
        projects_name = os.listdir(os.path.join(settings.MEDIA_ROOT, 'kino_dev/data'))
        ...
        for project_obj in data_project:
            ...
            path = os.path.join(settings.MEDIA_ROOT, 'kino_dev/data/'+name)
        ...
        if request.user is None or request.user.is_authenticated == False:
            ...
            return render(request, 'kino_dev/index.html', {'data_projects':data_project, 'projects':projects,'take_list':FolderPath.objects.order_by('path')})
...
def launch_preprocess(name, username):
    ...
    dir = name.split('.')[0]
    path = os.path.join(settings.MEDIA_ROOT, 'kino_dev/data/'+username)
    ...
    if request.method == 'POST' and request.FILES:
        ...
        if info == 'video':
            ...
            if launch_preprocess(filename, title.replace(" ", "_")):
                print('finished')
                return redirect('kino_dev:index')
            else:
                print('already')
                return render(request, 'kino_dev/upload.html', {'msg':'Already added','title':title})
        else:
            print('not video')
            return render(request, 'kino_dev/upload.html', {'msg':'Not a video','title':title})
    else:
        if request.method == 'POST':
            title = request.POST['title']
            return render(request, 'kino_dev/upload.html', {'title':title})
        return render(request, 'kino_dev/upload.html')

def preprocess(request):
    print(request)
    return render(request, 'kino_dev/process.html')
...
def edit_project(request):
    if request.method == 'POST':
        ...
        proj.title = title
        for root, dirs, files in os.walk(os.path.join(settings.MEDIA_ROOT, 'kino_dev/data/')):
            for d in dirs:
                if d == old_title:
                    os.rename(os.path.join(settings.MEDIA_ROOT, 'kino_dev/data/'+d), os.path.join(settings.MEDIA_ROOT, 'kino_dev/data/'+title_replace))
                    break
            break
        for root, dirs, files in os.walk(os.path.join(settings.MEDIA_ROOT, 'kino_dev/index_images/')):
            for d in dirs:
                if d == old_title:
                    os.rename(os.path.join(settings.MEDIA_ROOT, 'kino_dev/index_images/'+d), os.path.join(settings.MEDIA_ROOT, 'kino_dev/index_images/'+title_replace))
                    break
            break
...
def remove_project(request):
    if request.method == 'POST':
        title = request.POST['title']
        path = os.path.join(settings.MEDIA_ROOT, 'kino_dev/data/'+title)
...
def video_editing(request, id):
    ...
    # print(os.path.join(settings.BASE_DIR, 'kino_dev/static/kino_dev/data/', dir.path)+'/detections.json')
    ...
    if request.user is None or request.user.is_authenticated == False:
        print(settings.LOGIN_URL)
        return HttpResponseRedirect(settings.LOGIN_URL)
    return render(request, 'kino_dev/video_editing.html', {'id':id, 'title':split_title[1], 'part':part, 'path':dir.path, 'abs_path':dir.abs_path, 'width':width, 'height':height, 'frame_rate':round(frame_rate), 'next_id':next, 'prev_id':prev, 'owner':'None',
     'data_note':json.dumps(data_note), 'username':request.user.username, 'data_timelines':json.dumps(data_timelines)})
    # return render(request, 'kino_dev/index.html', {'image' : data_path})
...
def video_book(request, id):
    ...
    return render(request, 'kino_dev/video_book.html', {'username':request.user.username ,'crop':full_crop, 'full':full_full, 'tab':full_tab, 'path':full_list, 'json_shots':json.dumps(full_data), 'json_notes':json.dumps(full_note).replace('\"','\\"'), 'username':request.user.username})
...
def noting_app(request):
    return render(request, 'kino_dev/noting.html')
...
def fullhd(request):
    ...
    data_path = os.path.join(settings.MEDIA_ROOT, "kino_dev/image/")
    ...
    data_path = "/media/kino_dev/image/"+str(num)+".jpg"
    end = time.time()
    sec = (end-start)%60
...
def reframe(request):
    ...
    folder_path = os.path.join(settings.MEDIA_ROOT, 'kino_dev/data/out')
    ...
    if os.path.isfile(out_vid):
        response = HttpResponse(json.dumps({'src':'/media/kino_dev/data/output-video.mov'}))
        return response
    else:
        print('no file')
        return HttpResponse('')
...
def reframeCv(request):
    ...
    if os.path.isfile(reframe_final):
        response = HttpResponse(json.dumps({'src':'/media/kino_dev/data/reframe-audio.mp4'}))
        return response
    else:
        print('no file')
        return HttpResponse('')
